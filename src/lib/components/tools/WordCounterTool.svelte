...
<div class="space-y-4">
	...
	<div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
		...
		<StatTile label="Characters" value={stats.characters} hint="{stats.charactersNoSpaces} without spaces" />
		<StatTile label="UTF-8 bytes" value={stats.bytes} />
		<StatTile label="Reading time" value={stats.readingMinutes === 0 ? '—' : `~${stats.readingMinutes} min`} hint="at 220 wpm" />
		...
		<StatTile label="Avg word length" value={stats.words === 0 ? '—' : (stats.charactersNoSpaces / stats.words).toFixed(1)} />
	</div>
</div>
