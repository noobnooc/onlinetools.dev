...
const ja: Messages = {
	...
	ui: {
		...
		aboutH1: '電卓が勝手に本部へ通信すべきじゃない。',
		aboutLead:
			'たいていの「オンラインツール」サイトは、広告の壁で囲んだ入力欄に貼り付けた内容を、あなたが決して目にすることのないサーバーへこっそり送っています。どこかの適当なページで JWT をデコードして、一拍遅れて「たった今、自分のトークンを見知らぬ相手に渡してしまった」と気づく——そんなことにうんざりしました。onlinetools.dev はその答えです。毎日使う同じツール、ただし処理するコンピューターは、いまあなたの目の前にあるこれです。',
		aboutS1Head: 'すべてがあなたのマシンで動く',
		aboutS1Body:
			'ここにあるツールはどれも、ブラウザー内の純粋な計算です——往復通信もサーバーもなく、そもそもアップロードする先がありません。あなたの JSON、アクセストークン、先に消しておくつもりだったあの {env}:それらはクリップボードから画面へ渡り、そこで止まります。こうしたのは運用が安いから(実際に安い)ではなく、あなたのデータが私には関係ないからです。',
		aboutS2Head: '広告なし。トラッカーなし。アカウントなし。',
		aboutS2Body:
			'キー入力を数える分析スクリプトも、同意を求める cookie バナー(そもそも同意すべき cookie がありません)も、「続けるにはサインイン」も、三歩先で待つアップセルもありません。あなたが誰で何を貼り付けたか、私は本当に知りません——それが肝心です。ツールは仕事を終えたら、タブを閉じた瞬間にあなたを忘れるべきなのです。',
		aboutS3Head: 'ネットワークが落ちても動く',
		aboutS3Body:
			'ページを一度読み込めば、もうあなたのものです。サイト全体が静的で service worker にキャッシュされるので、飛行機の中でも、地下鉄でも、厳しく閉じられた社内プロキシの向こうでも、ツールは動き続けます。機内モードは JSON を整形するのにうってつけの場所です。',
		aboutS4Head: '速く、そして邪魔をしない',
		aboutS4Body:
			'スプラッシュ画面も、「規約に同意」も、あなたと作業の間に挟まるモーダルもありません。{kbd} を押せばどのツールにも飛べ、何かを貼れば適切なツールが自ら現れ、どの結果もクリップボードまでワンキーです。徹底してキーボードファースト。',
		aboutVerifyIntro:
			'プライバシーの宣言は安いものです——どのサイトも、あなたを売り渡す途中で「あなたのプライバシーを大切にします」と言います。だからここでは、信じてもらうためではなく、自分で確かめられるものを用意しました。このカウンターは、あなたのブラウザー自身のネットワーク活動をリアルタイムで見張っています:',
		aboutBugLine:
			'もしどれかのツールがあなたのデータを送ってはいけない場所へ送ったなら、それはビジネスモデルではなくバグです——{issue}してください、直します。',
		aboutBugLink: 'issue を報告',
